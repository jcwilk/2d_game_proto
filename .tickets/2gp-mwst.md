...
- [ ] Module header documents the **preset contract** and points to **`pipeline.mjs`** for orchestration.
- [ ] **Preset is the single source of truth** for dpad constants (`DPAD_FRAMES`, paths, sizes, `spriteRef`, etc.). **`dpad-workflow.mjs`** may still contain duplicates until **2gp-b4lm** — **deletion of duplicated constants from the monolith** is verified under **2gp-b4lm** (and parity under **2gp-f7b6**), not as a hard gate on closing **2gp-mwst** before **2gp-b4lm** lands.
- [ ] **`spriteRef`** matches **`individual-tiles`** (or documented grid variant) and **`src/art/atlasTypes.ts`** expectations per **2gp-s1lz**.
- [ ] If **2gp-98mn** did not add an automated **sheet + crop** test, this ticket’s tests or notes **close that gap** (smoke run or explicit justification).
