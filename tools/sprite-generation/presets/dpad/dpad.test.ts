...
describe("presets/dpad", () => {
  it("createPreset builds a runPipeline-ready object with gridFrameKeys spriteRef for art/dpad", () => {
    ...
    expect(p.frames).toStrictEqual(DPAD_FRAMES);
    ...
    expect(spriteRef.sheetImageRelativePath).toBe("art/dpad/sheet.png");
  ...
  it("throws without outBase", () => {
    expect(() => createPreset({} as import("./dpad.ts").CreateDpadPresetOpts)).toThrow(/outBase/);
  });
});
